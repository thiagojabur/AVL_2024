...
public class AVL {
	...
	private void updateHeight(Node node) {
		...
		if (node.leftNode == null && node.rightNode == null) // node is a leaf
	      node.height = 0;
	    else if (node.leftNode == null) // node não tem Subárvore esquerda
	    	node.height = 1 + ((Node)(node.rightNode)).height;
	    
	    else if (node.rightNode == null) // node não tem Subárvore direita
	...
	public void setBalanceFactors(Node root) {
		//visitar cada nó e fazer a conta 
		//FB = Altura (SAE) – altura(SAD)
